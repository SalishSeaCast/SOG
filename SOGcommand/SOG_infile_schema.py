...
class _RealDP(object):
    ...
    def deserialize(self, node, cstruct):
        if cstruct is colander.null:
            return colander.null
        if not isinstance(cstruct, six.string_types):
            raise colander.Invalid(
...
class _String(object):
   ...
   enclosed in double quotes.
    ...
    def serialize(self, node, appstruct):
        if appstruct is colander.null:
            return colander.null
        if not isinstance(appstruct, six.string_types):
            raise colander.Invalid(
...
class SOG_Infile(colander.MappingSchema):
    ...
    vary_temperature_fixed = _SOG_Boolean(name='vary%temperature%fixed')
    vary_temperature_value = _SOG_RealDP(name='vary%temperature%value')
    vary_temperature_shift = _SOG_RealDP(name='vary%temperature%shift')
    ...
    northern_water_power_riverflow_influence = _SOG_RealDP(
        name='power_northern')
    northern_water_normalization_riverflow_influence = _SOG_RealDP(
        name='normal_northern')
...
SOG_KEYS = [
    ...
    'river_TA_record', 'river_TA', 'river_pH',
...
SOG_EXTRA_KEYS = {
    ...
    'vary%temperature%enabled': {
        '.true.': ['vary%temperature%fixed'],
        '.false.': [],
    },
    'vary%temperature%fixed': {
